...
import { createTimePeriod } from './common.js';

test('create a TimePeriod and get all time periods', async () => {
  await runScenario(async scenario => {
    ...
    const createRecord: Record = await createTimePeriod(alice.cells[0]);
    assert.ok(createRecord);
    
    await dhtSync([alice, bob], alice.cells[0].cell_id[0]);
    
    // Bob gets all time periods again
