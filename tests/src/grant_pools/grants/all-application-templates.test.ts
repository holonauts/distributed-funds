...
import { createApplicationTemplate } from './common.js';

test('create a ApplicationTemplate and get all application templates', async () => {
  await runScenario(async scenario => {
    ...
    // Bob gets all application templates
    ...
    const createRecord: Record = await createApplicationTemplate(alice.cells[0]);
    assert.ok(createRecord);
    
    await dhtSync([alice, bob], alice.cells[0].cell_id[0]);
    
    // Bob gets all application templates again
