<script lang="ts">
	...
	let amount: bigint;
	let renderApi: typeof RenderAPI | undefined = undefined;
	...
	async function saveDraft() {
		...
		try {
			...
			goto(
				`/grants-pool/${$page.params.actionHashB64}/applications/${encodeHashToBase64(record.signed_action.hashed.hash)}`
			);
		} catch (e) {
	...
	<svelte:fragment let:entry>
		...
		>
			<svelte:fragment let:entry={applicationTemplate}>
				<BaseBreadcrumbs
					title="Apply"
					replacements={{ [$page.params.actionHashB64]: entry.name }}
				/>
