...
<div style="display: flex; flex-direction: column">
	...
	<div style="margin-bottom: 16px">
		<vaadin-date-time-picker
			label="Start At"
			value={new Date(startAt / 1000).toISOString()}
			on:change={(e) => {
				startAt = new Date(e.target.value).valueOf() * 1000;
			}}
			required
		></vaadin-date-time-picker>
	</div>
	...
	<div style="margin-bottom: 16px">
		<vaadin-date-time-picker
			label="End At"
			value={new Date(endAt / 1000).toISOString()}
			on:change={(e) => {
				endAt = new Date(e.target.value).valueOf() * 1000;
			}}
			required
		></vaadin-date-time-picker>
	</div>
