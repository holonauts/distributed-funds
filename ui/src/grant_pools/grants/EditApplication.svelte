...
let currentApplication: Application = decode((currentRecord.entry as any).Present.entry) as Application;
...
onMount(() => {
  ...
  if (originalApplicationHash === undefined) {
    throw new Error(`The originalApplicationHash input is required for the EditApplication element`);
  }
});
