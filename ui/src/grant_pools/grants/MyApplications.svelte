<script lang="ts">
	...
	onMount(async () => {
		if (author === undefined) {
			throw new Error(`The author input is required for the MyApplications element`);
		}
...
{:else if hashes.length === 0}
	<span>No applications found for this author.</span>
{:else}
	<div style="display: flex; flex-direction: column">
		{#each hashes as hash}
			<div style="margin-bottom: 8px;">
				<ApplicationDetail applicationHash={hash} on:application-deleted={() => fetchApplications()}
				></ApplicationDetail>
			</div>
		{/each}
	</div>
