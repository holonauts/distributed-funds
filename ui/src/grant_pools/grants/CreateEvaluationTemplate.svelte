...
onMount(() => {
  if (qualitativeJsonSchema === undefined) {
    throw new Error(`The qualitativeJsonSchema input is required for the CreateEvaluationTemplate element`);
  }
  if (quantitativeRating === undefined) {
    throw new Error(`The quantitativeRating input is required for the CreateEvaluationTemplate element`);
  }
});
...
async function createEvaluationTemplate() {  
  ...
  try {
    ...
    dispatch('evaluation-template-created', { evaluationTemplateHash: record.signed_action.hashed.hash });
  } catch (e) {
